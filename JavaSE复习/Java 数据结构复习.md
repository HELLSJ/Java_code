...
Map和Set是动态查找的容器，方便我们在查找时进行一些插入和删除的操作
### Map 是什么？
...
![](image/img-20251217-2.png)
### 哈希表是什么？

- 用 **哈希函数** 把 Key 映射到数组下标
- 理想情况：一次定位，直接命中
#### 哈希函数

不经过比较，一次直接从表中得到要搜索的元素，通过该元素的存储位置和它的关键码之间建立一一映射的关系
这种函数叫做**哈希函数：hash(key) = key % capacity**
capacity是存储元素底层空间的大小

![](https://i-blog.csdnimg.cn/blog_migrate/2a6d64065b52f2f30b9f66cb479faa46.png)
### 哈希冲突

 **哈希冲突**：不同的关键字key，通过相同的哈希函数，得到相同的值

![](https://i-blog.csdnimg.cn/blog_migrate/895fa5505690ae3f0e66b644569788ea.png)

哈希冲突无法解决，只能降低冲突率
#### 冲突率由什么来衡量？（负载因子）

$$\text{负载因子} = \frac{\text{当前存储的元素个数}}{\text{哈希表的容量}}$$
**控制哈希冲突的程度**
...
```
1. 创建新数组 newArray（长度 = 2 * oldLength）
2. 遍历旧数组每个桶 array[i]
3. 对桶里的链表逐节点取出来  
4. 对每个节点重新计算新下标 newIndex  
5. 把节点插入新数组对应桶（常用头插/尾插都行）
6. 最后 array = newArray
```
### 解决方法
...
也叫开放定址法，当发生哈希冲突时，如果哈希表未被装满，说明在哈希表中还存在未填满的位置，那么可以把key存放到冲突位置的“下一个”空位置中去
##### 1. 线性探测法
...
**优点**：1. 实现简单；2. 连续内存，缓存友好
**缺点**：1. **容易产生“堆积（主聚集）”**；2. 冲突越多，查找越慢
...
- 24 → hash = 4（冲突）  
    ...
    → 4 + 2² = 8 → 空 → 放 8
优点：**减少堆积问题**；比线性探测更均匀
缺点：实现稍复杂；不能保证一定找到空位（需要表长 & 负载因子配合）
...
![](JavaSE复习/image/img-20251217.png)
![](JavaSE复习/image/img-20251217-1.png)
...
反射是 Java 在运行时获取类信息，并动态创建对象、访问属性、调用方法的机制。
...
- 创建对象（`newInstance` / 构造器）
- 访问私有属性（`setAccessible(true)`）
- 调用私有方法
- 操作注解
...
枚举是一组固定、有限、类型安全的常量集合

**枚举本质是一个特殊的类，继承自 `java.lang.Enum`**
- 构造方法 **默认 private**
- 外部 **不能 new 枚举对象**
- 每个枚举值都是 **static final 对象**


**能不能用反射创建枚举对象？**  
**不能**

原因：不能使用反射创建枚举对象。虽然枚举本质是一个类，但 JVM 在 Constructor.newInstance 阶段对 enum 做了**强制校验**，如果尝试通过反射创建枚举实例，会直接抛出 IllegalArgumentException。这样设计是为了保证枚举实例数量固定、单例语义不被破坏，因此 enum 也是最安全的单例实现方式。

**为什么 JVM 要“专门针对枚举”这样做？**
- 枚举的实例数量必须是“固定的”
- 枚举必须天然是“单例的”
- 枚举是“语言级特性”，不是普通类
## Lambda

Lambda用来简化函数式接口实现的语法糖
本质:
- 不是函数
- 是 **对象**
- 实现了 **函数式接口**(一个接口有且只有一个抽象方法)

Lambda 使用前提：接口中有且只有一个抽象方法
